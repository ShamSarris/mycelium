...
/**
 * The suite that spends real money.
 *
 ...
 * a fake cannot tell you that the SDK's request shape is accepted by the real
 * API, that usage comes back populated, or that containment holds up against
 * a real, completing model call.
 *
 * Needs `WORKER_LIVE_TESTS=1` and a real `MODEL_API_KEY`. It is not part of
 * the default run and never will be.
 *
 * Ticket 14: this file used to open with a "raw transport" suite exercising
 * `AnthropicTransport` directly (usage shape, prompt caching, a normalized
 * tool call) — `HostLoopRunner`'s own seam, now deleted along with it. The
 * Agent SDK owns the equivalent request/response shape internally; there is
 * no longer a comparable seam in this package for a test to call directly,
 * so that coverage is gone, not relocated. What is left below already spends
 * real money through the one surviving runner and is a strictly stronger
 * check for it (a real subprocess and a real completing task, not just a
